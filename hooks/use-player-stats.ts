  ...
  const playerStats = useMemo(() => {
    const stats: PlayerStats[] = players.map((player) => {
      ...
      const attendedSessions = completedSessions.filter((session) =>
        session.responses.some((response) => response.playerId === player.id && response.status === "coming"),
      )
      ...
      return {
        ...
        attendanceRate: playerSessions.length > 0 ? (attendedSessions.length / playerSessions.length) * 100 : 0,
      }
    })
